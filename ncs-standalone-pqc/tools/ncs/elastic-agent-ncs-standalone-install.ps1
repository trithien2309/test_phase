...
function Test-TcpPort {
    ...
    try {
        ...
        if ($result) {
            ...
            Write-Warn "TCP endpoint is not reachable now: ${HostName}:${Port}. Continuing so Agent can retry later."
        }
    } catch {
...
inputs:
  - id: ncs-windows-events
    ...
    data_stream:
      namespace: default
    streams:
      - id: ncs-winlog-security
        name: Security
        data_stream:
          type: logs
          dataset: ncs.windows.security
        ignore_older: 72h
      - id: ncs-winlog-windows-powershell
        name: Windows PowerShell
        data_stream:
          type: logs
          dataset: ncs.windows.powershell
        ignore_older: 72h
      - id: ncs-winlog-powershell-operational
        name: Microsoft-Windows-PowerShell/Operational
        data_stream:
          type: logs
          dataset: ncs.windows.powershell_operational
        ignore_older: 72h

  - id: ncs-transport-smoke-file
    type: filestream
    use_output: default
    data_stream:
      namespace: default
    streams:
      - id: ncs-transport-smoke
        data_stream:
          type: logs
...
Write-Host "  NCS Elastic Agent PQC Standalone Installer" -ForegroundColor Cyan
Write-Host "====================================================" -ForegroundColor Cyan
...
Write-Phase "[4/8] Prepare Elastic Agent standalone package"
$elasticAgentExe = Ensure-AgentPackageLayout -AgentExe $elasticAgentExe -FilebeatPqcExe $filebeatPqcExe
Write-StandaloneConfig -ConfigPath (Join-Path $agentDir "elastic-agent.yml") -LogstashHost $logstashHost -SmokeLog $SmokeLogPath
...
Write-Phase "[7/8] Prepare smoke log and install Elastic Agent standalone service"
New-Item -ItemType Directory -Force (Split-Path -Parent $SmokeLogPath) | Out-Null
$padding = "A" * 1600
Set-Content -Path $SmokeLogPath -Value "ncs-standalone-smoke-bootstrap $(Get-Date -Format o) $padding" -Encoding UTF8
Add-Content -Path $SmokeLogPath -Value "ncs-standalone-smoke-event $(Get-Date -Format o) $padding" -Encoding UTF8
Write-OK "Smoke log prepared: $SmokeLogPath"
...
Write-Host ""
Write-Host "Append another smoke event with:" -ForegroundColor Cyan
Write-Host "  Add-Content $SmokeLogPath `"ncs-standalone-manual-event `$(Get-Date -Format o)`""
Write-Host ""
...
Write-Host "  Search: data_stream.dataset : `"ncs.transport_smoke`" or event.code : `"4688`" or event.code : `"4104`""

Write-Host ""
Write-Host "NCS Elastic Agent PQC standalone install flow completed." -ForegroundColor Green
